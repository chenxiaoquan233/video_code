...
public:
	...
	int bin_to_text(char* _output_text_path,char * _check_file_path);
	...
	int decode(char* _input_video_path, char* _output_text_path, char* _check_file_path);
	int find_Qr_anchor(Mat& srcImg, vector<vector<Point>>& qrPoint);
	unsigned int GetFEC(unsigned int CX);
	unsigned int CorrectError(unsigned int code, FILE* check_file);
	void CreateCheckMatrix();
	void enter_text();
	bool check_text_state();
	void num_or_text(char ch, FILE* output_file);
	void Parameterjudgement(char* _input_file_name, char* _output_file_name);
};
